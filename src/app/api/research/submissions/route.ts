...
const RESEARCH_CATEGORIES = ['RESEARCH', 'PAPERS_PUBLISHED', 'BOOKS_AUTHORED', 'PATENTS', 'ARTICLES'];
...
export async function PATCH(req: NextRequest) {
    ...
    // Verify the activity is a research category
    const existing = await prisma.activity.findUnique({ where: { id } });
    if (!existing) return NextResponse.json({ error: 'Activity not found' }, { status: 404 });
    if (!RESEARCH_CATEGORIES.includes(existing.category)) {
        return NextResponse.json({ error: 'This activity is not a research category' }, { status: 403 });
    }
    ...
    await prisma.auditLog.create({
        data: {
            ...
            reason: comment || 'R&D coordinator validation',
            role: 'RND_COORDINATOR',
        },
    });
